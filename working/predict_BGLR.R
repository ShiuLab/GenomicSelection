...
for(i in 1:length(Y)){
  ...
  ll <- fm$fit$logLikAtPostMean   # log-likelihood evaluated at posterior mean
  K <- fm$fit$pD    # estimated effective number of parameters, Spiegelhalter et al. (2002).
  ...
  res <- data.frame(BGLR_model, X_file, save_name, names(Y)[i], ho_name, feat_method, feat_num, pcc, pcc^2, mse, aic, bic, time.taken)
